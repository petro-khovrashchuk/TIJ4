//:Reusing Classes Ex.24

/**
 * Exercise 24: (2) In Beetle.java, inherit a specific type of beetle from class Beetle, following the same format as
 * the existing classes. Trace and explain the output.
 *
 * Created by Petr on 17/03/2016.
 */

import static net.mindview.util.Print.*;
class Insect {
    private int i = 9;
    protected int j;
    Insect() {
        print("i = " + i + ", j = " + j);
        j = 39;
    }
    private static int x1 =
            printInit("static Insect.x1 initialized");
    static int printInit(String s) {
        print(s);
        return 47;
    }
}
class Beetle extends Insect {
    private int k = printInit("Beetle.k initialized");
    public Beetle() {
        print("k = " + k);
        print("j = " + j);
    }
    private static int x2 =
            printInit("static Beetle.x2 initialized");
    public static void main(String[] args) {
        print("Beetle constructor");
        Beetle b = new Beetle();
    }
}
public class Ladybug extends Beetle {
    public Ladybug() {
        print("k = " + k);
        print("j = " + j);
    }
    private int k = printInit("Ladybug.k initialized"); /* standard data fields are processed before constructor of the
                                                         * same class */
    private static int x3 = printInit("static Ladybug.x3 initialized");
    public static void main(String[] args) {
        print("Ladybug constructor");
        Ladybug l = new Ladybug();
    }
}